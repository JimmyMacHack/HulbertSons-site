...
const AIChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: "Hello! I'm the Hulbert & Sons virtual assistant. How can I help you with your home project today?" }
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: `You are the friendly and professional AI assistant for Hulbert & Sons Home Services. 
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-[350px] sm:w-[400px] h-[500px] bg-white rounded-3xl shadow-2xl border border-brandGold/20 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
          {/* Header */}
          <div className="bg-slate-900 p-4 flex items-center justify-between border-b border-brandGold/30">
            <div className="flex items-center">
              <div className="w-10 h-10 bg-brandGold rounded-full flex items-center justify-center mr-3 shadow-lg">
                <ShieldCheck className="w-6 h-6 text-slate-900" />
              </div>
              <div>
                <h3 className="text-white font-black text-sm uppercase tracking-wider">Hulbert Assistant</h3>
                <div className="flex items-center">
          ...
          <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-100 flex items-center gap-2">
            <input 
              ...
              className="flex-1 bg-slate-100 border-none focus:ring-2 focus:ring-brandGold rounded-xl px-4 py-3 text-sm font-medium outline-none transition-all"
            />
            <button 
              type="submit"
              disabled={!input.trim() || isLoading}
              className="bg-slate-900 text-brandGold p-3 rounded-xl hover:bg-brandGold hover:text-slate-900 disabled:opacity-50 disabled:hover:bg-slate-900 disabled:hover:text-brandGold transition-all shadow-md active:scale-95"
            >
      ...
      <button 
        ...
        className={`bg-slate-900 text-white p-4 rounded-full shadow-2xl hover:bg-slate-800 transition-all hover:scale-110 border-2 border-brandGold flex items-center justify-center group ${!isOpen ? 'animate-bounce-slow' : ''}`}
