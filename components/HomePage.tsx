...
import { CheckCircle, Star, Clock, HardHat, ChevronLeft, ChevronRight, ArrowRight, Video, Phone, Mail, PaintBucket, Hammer, MapPin, ChevronDown, Repeat, Camera } from 'lucide-react';
    ...
    <div className="w-full bg-white py-12 border-b border-slate-100 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 mb-8 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-brandGold/10 rounded-lg">
            <Camera className="w-5 h-5 text-brandGold" />
          </div>
          <h2 className="text-xs md:text-sm font-black uppercase tracking-[0.3em] text-slate-900 italic">
            Recent Completed <span className="text-brandGold">Jobs</span>
          </h2>
        </div>
        <div className="hidden sm:flex items-center gap-2">
          <span className="w-2 h-2 bg-brandGold rounded-full animate-pulse"></span>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Real Results</span>
        </div>
      </div>
      ...
      <div className="relative flex">
        <div className="flex animate-marquee whitespace-nowrap gap-6 px-4">
          {displayImages.map((src, idx) => (
            <div 
              key={idx} 
              className="relative w-64 md:w-80 h-48 md:h-64 rounded-2xl overflow-hidden shadow-xl border border-slate-200 group flex-shrink-0"
            >
              <img 
                src={src} 
                alt={`Recent Job ${idx}`} 
              ...
              <div className="absolute bottom-4 left-4 transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                <span className="bg-brandGold text-slate-900 text-[10px] font-black uppercase px-3 py-1 rounded-full shadow-lg">
                  Job Completed
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
            ...
            >
              <div className="relative w-full max-w-lg h-full transition-all duration-700 select-none">
                {/* Before Image Card */}
                <div className={`absolute transition-all duration-1000 ease-in-out rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white
                  ${isAfterFront 
                    ? 'top-4 right-24 w-80 h-[420px] -rotate-6 grayscale-[0.6] z-0 opacity-70 scale-90 translate-x-[-20%]' 
                    : 'bottom-0 right-0 w-[420px] h-[500px] rotate-0 grayscale-0 z-20 opacity-100 scale-100 shadow-slate-900/50'
                  }`}
                >
                  <img 
                ...
                <div className={`absolute transition-all duration-1000 ease-in-out rounded-[2rem] overflow-hidden shadow-2xl border-4 border-brandGold
                  ${isAfterFront 
                    ? 'bottom-0 right-0 w-[420px] h-[500px] rotate-0 z-20 opacity-100 scale-100 shadow-brandGold/40' 
                    : 'top-4 right-24 w-80 h-[420px] rotate-6 z-0 opacity-70 scale-90 translate-x-[-20%] grayscale-[0.2]'
                  }`}
                >
                  <img 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-10 max-w-6xl mx-auto">
            ...
            <RevealOnScroll delay={100} className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden border border-slate-200 group hover:-translate-y-2">
              ...
              <div className="p-8">
                <p className="text-slate-900 mb-8 text-lg leading-relaxed font-black">
                ...
                <button onClick={() => navigateTo('services')} className="w-full bg-slate-900 text-brandGold py-4 rounded-xl font-black uppercase tracking-widest border border-slate-900 hover:bg-brandGold hover:text-slate-900 transition-all flex items-center justify-center group shadow-xl shadow-slate-900/20">
