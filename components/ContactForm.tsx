...
A client has written this project description: "${projectDetails}". ${selectedImage ? "They have also provided a photo of the issue." : ""} Analyze this request and provide a detailed estimate and professional advice.` }
  ...
  if (isSubmitted) {
    return (
      <div className="bg-white rounded-3xl p-12 text-center shadow-2xl border border-brandGold/20 animate-in fade-in zoom-in-95">
        ...
        <h3 className="text-3xl font-black text-slate-900 mb-4 uppercase italic">Request Sent!</h3>
  ...
  return (
    <div className="bg-slate-300/40 backdrop-blur-md rounded-3xl shadow-2xl p-8 border border-white/20 hover:shadow-brandGold/5 transition-shadow duration-500">
      <div className="flex items-center justify-between mb-8">
        <div className="bg-slate-900/10 px-4 py-2 rounded-xl flex items-center gap-2">
          <Clock className="w-4 h-4 text-slate-900" />
          <span className="text-[10px] font-black uppercase tracking-wider text-slate-900">Est. turnaround: &lt;24 Hours</span>
        </div>
        <div className="text-[10px] font-black uppercase tracking-wider text-slate-500">hello@hulbertsons.com</div>
      </div>
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input 
              required
              type="text" 
              className="w-full px-5 py-4 rounded-xl bg-white border border-slate-200 focus:border-brandGold focus:ring-4 focus:ring-brandGold/10 transition-all outline-none shadow-sm text-slate-900 font-medium placeholder:text-slate-300" 
              placeholder="John" 
            />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-black text-slate-700 mb-2 uppercase tracking-widest">Phone Number</label>
            <input 
              required
              type="tel" 
              className="w-full px-5 py-4 rounded-xl bg-white border border-slate-200 focus:border-brandGold focus:ring-4 focus:ring-brandGold/10 transition-all outline-none shadow-sm text-slate-900 font-medium placeholder:text-slate-300" 
        ...
        <div className="relative">
          <div className="flex justify-between items-end mb-2">
            <label className="block text-sm font-black text-slate-700 uppercase tracking-widest">Project Details</label>
            {!aiSuggestion && (
              <button 
                type="button" 
          ...
          <textarea 
            ...
            className="w-full px-5 py-4 rounded-2xl bg-white border border-slate-200 focus:border-brandGold focus:ring-4 focus:ring-brandGold/10 transition-all outline-none resize-none shadow-sm text-slate-900 font-medium placeholder:text-slate-300" 
        ...
        <div className="space-y-2">
          <label className="block text-sm font-black text-slate-700 uppercase tracking-widest">Attach Photo (Optional)</label>
          <div 
            ...
            className={`relative border-2 border-dashed rounded-2xl transition-all flex flex-col items-center justify-center p-6 cursor-pointer group
              ${selectedImage ? 'border-brandGold bg-white' : 'border-slate-300 hover:border-brandGold bg-white/50 hover:bg-white'}
            `}
          >
            <input 
              type="file" 
            ...
            {selectedImage ? (
              ...
              <>
                <div className="w-12 h-12 bg-slate-900/5 rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <Upload className="w-6 h-6 text-slate-400 group-hover:text-brandGold" />
                </div>
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest group-hover:text-brandGold transition-colors">
                  Upload Project Photo
                </p>
                <p className="text-[10px] text-slate-400 mt-1">Faster estimates with visual context</p>
              </>
            )}
          </div>
        </div>
        ...
        {aiSuggestion && (
          <div className="bg-white/90 backdrop-blur-sm border-2 border-brandGold rounded-2xl p-6 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-500">
            {aiSuggestion.urgency === 'High' && (
              <div className="bg-red-50 border border-red-100 text-red-800 p-4 rounded-xl mb-6 flex items-start">
                <div className="bg-red-500 p-1.5 rounded-full mr-3 mt-0.5">
                  <Phone className="w-4 h-4 text-white" />
                </div>
                <div>
                  <p className="font-black text-xs uppercase tracking-[0.2em] text-red-600 mb-1">High Urgency Detected</p>
            ...
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center">
                ...
                <h4 className="font-black text-slate-900 text-sm uppercase tracking-widest italic">AI Insights</h4>
              </div>
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-700 bg-slate-50 p-4 rounded-xl border border-slate-100 italic font-medium leading-relaxed">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 bg-brandGold hover:bg-slate-900 text-slate-900 hover:text-white text-xs font-black uppercase tracking-widest py-3.5 rounded-xl transition-all shadow-lg active:scale-95"
              >
                Use This Suggestion
              </button>
              <button 
                ...
                className="px-6 py-3.5 text-slate-400 hover:text-slate-900 text-xs font-black uppercase tracking-widest transition-colors"
              >
                Dismiss
              </button>
            </div>
        ...
        <div className="space-y-4 pt-4">
          <button 
            type="submit" 
            className="w-full bg-brandGold hover:bg-slate-900 text-slate-900 hover:text-white font-black uppercase tracking-[0.25em] py-5 rounded-xl shadow-2xl transition-all transform hover:-translate-y-1 active:scale-95 text-lg flex items-center justify-center gap-3"
          >
            Submit Request
          </button>
          <div className="flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 opacity-60">
